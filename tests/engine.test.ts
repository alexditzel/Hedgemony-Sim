...
function startRedPhase(): GameState {
  ...
  const red = beginRedInvestmentsAndActions(actionPhase.state, [
    "RU",
    "PRC",
    "DPRK",
    "IR",
...
describe("turn sequence and Red signaling", () => {
  ...
  it("keeps each Red player's revealed cards separate and exposes remaining choices plus skip", () => {
    let state = recordGameStartSummary(freshState(), "Opening summary.");
    let result = signalRedCards(
      state,
      "RU",
      ["RU-ACT-01", "RU-ACT-02", "RU-INV-01"],
      "RU brief.",
    );
    expect(result.issues).toHaveLength(0);
    state = result.state;
    result = signalRedCards(
      state,
      "PRC",
      ["PRC-ACT-01", "PRC-ACT-02", "PRC-INV-01"],
      "PRC brief.",
    );
    state = result.state;
    ...
    expect(
      getRedChoiceOptions(state, "RU").remaining.map((card) => card.id),
    ).toEqual(["RU-ACT-01", "RU-ACT-02", "RU-INV-01"]);
  ...
  it("enforces Red signaling mix when three cards are chosen", () => {
    ...
    expect(result.issues).toHaveLength(0);
    const invalid = signalRedCards(
      state,
      "RU",
      ["RU-ACT-01", "RU-ACT-02", "RU-ACT-01"],
      "Duplicate.",
    );
  ...
  it("requires a played Red two-card set to contain an Action and an Investment before skipping", () => {
    ...
    let result = playRedSignaledCard(
      state,
      "PRC",
      "PRC-ACT-01",
      new SequenceDiceRoller([5]),
    ...
    expect(skip.issues[0].message).toContain(
      "at least one Action and one Investment",
    );
  });
  ...
  it("prevents a Red player who signaled two Actions from playing both and getting stuck", () => {
    ...
    const first = playRedSignaledCard(
      state,
      "PRC",
      "PRC-ACT-01",
      new SequenceDiceRoller([5]),
    ...
    expect(getRedChoiceOptions(state, "PRC")).toMatchObject({
      remaining: [],
      canSkip: true,
    });
    const second = playRedSignaledCard(
      state,
      "PRC",
      "PRC-ACT-02",
      new SequenceDiceRoller([5]),
  ...
  it("charges a flat Red additional-card cost for each card after the first", () => {
    ...
    const secondCard = resolveCard(
      afterOnePlayed,
      { acting_player_id: "RU", card_id: "RU-INV-01" },
      new SequenceDiceRoller([5]),
    );
    expect(
...
describe("readiness, resources, and movement", () => {
  ...
  it("enforces Blue investments before Blue actions", () => {
    ...
    const earlyRedPhase = beginRedInvestmentsAndActions(paid, [
      "RU",
      "PRC",
      "DPRK",
      "IR",
    ]);
    expect(earlyRedPhase.issues[0].message).toContain(
      "Blue investments must be ended",
    );
    ...
    expect(prematureAction.issues[0].message).toContain(
      "until Blue investments are complete",
    );
    expect(prematureAction.state).toBe(paid);

    const actionPhase = advanceBlueToActions(paid).state;
    const lateInvestment = resolveCard(
      actionPhase,
      { acting_player_id: "US", card_id: "US-INV-01" },
      new SequenceDiceRoller([5]),
    );
    expect(lateInvestment.issues[0].message).toContain(
      "after Blue actions have begun",
    );
  ...
  it("requires explicit White Cell Red sequence input under default rules", () => {
    ...
    expect(missingSequence.issues[0].message).toContain(
      "White Cell must choose",
    );
    expect(missingSequence.state).toBe(actionPhase);
    expect(missingSequence.state.pending_adjudications).toHaveLength(0);

    const invalidSequence = beginRedInvestmentsAndActions(actionPhase, [
      "RU",
      "RU",
      "PRC",
      "IR",
    ...
    const chosenSequence = beginRedInvestmentsAndActions(actionPhase, [
      "PRC",
      "RU",
      "IR",
      "DPRK",
    ...
    expect(chosenSequence.state.red_sequence).toEqual([
      "PRC",
      "RU",
      "IR",
      "DPRK",
  ...
  it("uses D10 rolls for optional random Red sequence and requires a roller", () => {
    ...
    const randomized = beginRedInvestmentsAndActions(
      actionPhase,
      undefined,
      new SequenceDiceRoller([8, 1, 5, 3]),
    );
    expect(randomized.issues).toHaveLength(0);
    expect(randomized.state.red_sequence).toEqual(["PRC", "IR", "DPRK", "RU"]);
    expect(
      randomized.state.rolls.slice(-4).map((roll) => roll.formula),
    ).toEqual(["D10(8) = 8", "D10(1) = 1", "D10(5) = 5", "D10(3) = 3"]);
  });
  ...
  it("accounts for the 11 FF CONUS 100% readiness subtotal through scenario table extensions", () => {
    ...
    const bill = calculateUsReadinessBill(state);
    expect(bill.issues).toHaveLength(0);
    expect(bill.rows).toContainEqual({
      location: "CONUS",
      readiness: 100,
      force_factors: 11,
      cost: 11,
  ...
  it("buys back readiness using location-specific printed costs", () => {
    const state = {
      ...freshState(),
      phase: "BlueInvestmentsAndActions" as const,
    };
  ...
  it("does not allocate annual resources outside the annual allocation phase", () => {
    ...
    expect(result.issues[0].message).toContain(
      "Expected phase AnnualResourcesAllocation",
    );
...
describe("combat resolution and card effects", () => {
  ...
  it("filters private logs and rolls by viewer", () => {
    ...
    const privateLog = result.state.event_log.find(
      (entry) => entry.visibility === "private_to_player_and_white_cell",
    );
    const privateRoll = result.state.rolls.find(
      (roll) => roll.visibility === "private_to_player_and_white_cell",
    );
  ...
  it("requests White Cell adjudication for Blue free play and White Cell cards", () => {
    const paid = payUsReadinessBill(startTurnThroughBlueReadiness()).state;
    const requested = requestBlueFreePlayAdjudication(
      paid,
      "US",
      "Build a base with partner access.",
    );
    expect(requested.state.pending_adjudications.at(-1)?.reason).toContain(
      "Blue free-play request",
    );
    const actionPhase = advanceBlueToActions(paid).state;
    const card = resolveCard(
      actionPhase,
      { acting_player_id: "US", card_id: "US-ACT-02" },
      new SequenceDiceRoller([5]),
    );
  ...
  it("allows zero-cost White Cell event injection during the Blue turn", () => {
    const paid = payUsReadinessBill(startTurnThroughBlueReadiness()).state;
    const result = injectWhiteCellEvent(
      paid,
      "INT-EVT-01",
  ...
  it("pauses for adjudication when a card D10 table has no matching outcome row", () => {
    ...
    const result = resolveCard(
      gapState,
      { acting_player_id: "DPRK", card_id: "DPRK-ACT-01" },
      new SequenceDiceRoller([9]),
    );
  ...
  it("blocks Critical Capability upgrades that exceed National Tech Level", () => {
    const paid = payUsReadinessBill(startTurnThroughBlueReadiness()).state;
    const firstUpgrade = resolveCard(
      paid,
      { acting_player_id: "US", card_id: "US-INV-01" },
      new SequenceDiceRoller([5]),
    );
    ...
    expect(blockedUpgrade.issues[0].message).toContain(
  ...
  it("resolves scenario cards with deterministic effects, future effects, and frequency limits", () => {
    ...
    const ukraineSupport = resolveCard(
      actionPhase,
      { acting_player_id: "US", card_id: "BLUE-01" },
      new SequenceDiceRoller([5]),
    );
    expect(
    ...
    const missileTest = resolveCard(
      actionPhase,
      { acting_player_id: "US", card_id: "US-10" },
      new SequenceDiceRoller([3]),
    );
    expect(
    ...
    const infrastructureAttack = resolveCard(
      actionPhase,
      { acting_player_id: "US", card_id: "UAC-1" },
  ...
  it("supports Red scenario cards that use RT B, ransomware tables, and card-defined limits", () => {
    ...
    const ransomware = resolveCard(
      state,
      { acting_player_id: "DPRK", card_id: "DPRK-30" },
    ...
    expect(repeatedRansomware.issues[0].message).toContain(
      "once per two turns",
    );

    const malware = resolveCard(
      state,
  ...
  it("resolves representative standard cards without adding rule exceptions", () => {
    ...
    const c4isrUpgrade = resolveCard(
      state,
      { acting_player_id: "PRC", card_id: "PRC-24" },
      new SequenceDiceRoller([2]),
    );
    expect(
    ...
    expect(
      grayZone.issues.filter((issue) => issue.severity === "error"),
    ).toHaveLength(0);
    ...
    expect(
      iranSupport.issues.filter((issue) => issue.severity === "error"),
    ).toHaveLength(0);
    ...
    const peacekeeping = injectWhiteCellEvent(
      state,
      "EVT-CON-02",
      "Peacekeeping crisis.",
      new SequenceDiceRoller([8]),
    );
    expect(
      peacekeeping.issues.filter((issue) => issue.severity === "error"),
    ).toHaveLength(0);
  ...
  it("injects scenario event cards and applies supported event effects", () => {
    ...
    const natoEvent = injectWhiteCellEvent(
      state,
      "EVT-NATO-18",
      "Nordic accession.",
      new SequenceDiceRoller([5]),
    ...
    const coup = injectWhiteCellEvent(
      state,
      "EVT-IR-11",
      "Coup.",
      new SequenceDiceRoller([5]),
...
describe("force development, proxies, reset, and annual allocation", () => {
  ...
  it("resolves proxy participation and proxy development consequences", () => {
    const participation = resolveProxyParticipation(
      freshState(),
      "IR-PROXY-01",
      "Medium",
      new SequenceDiceRoller([4]),
  ...
  it("rolls annual resources with inline formula and advances to summary", () => {
    const state = {
      ...freshState(),
      phase: "AnnualResourcesAllocation" as const,
    };
    const result = annualResourceAllocation(state, new SequenceDiceRoller([8]));
