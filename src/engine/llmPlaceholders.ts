import type { AdjudicationRequest, CardId, GameState, PlayerId, RedSignalState } from "./types";
...
export function placeholderRedSignalDecision(state: GameState, playerId: PlayerId): PlaceholderRedSignalDecision {
  ...
  return {
    ...
    activationIntent: Object.fromEntries(cardIds.map((id) => [id, "Undeclared"]))
  };
}

export function placeholderRedPlayDecision(state: GameState, playerId: PlayerId): PlaceholderRedPlayDecision {
...
export function placeholderWhiteCellSummary(kind: "game_start" | "state_of_world", turn: number): string {
...
export function placeholderWhiteCellAdjudicationResolution(request: AdjudicationRequest): string {
...
export function placeholderWhiteCellEventDecision(state: GameState): CardId | undefined {
  // Placeholder for a future White Cell LLM request that will decide whether to inject an event and which event to use.
  return Object.values(state.cards).find((card) => card.type === "InternationalEvent" || card.type === "DomesticEvent")?.id;
}
