...
export interface ResolveCardArgs {
  acting_player_id: PlayerId | "WhiteCell";
  card_id: CardId;
...
function visibilityForCard(card?: Card): Visibility {
  return card?.public_private === "Private"
...
function addAdjudication(
  ...
  ruleRefs: string[],
  payload?: JsonValue,
  requestedBy?: PlayerId | "WhiteCell",
  cardId?: CardId,
): AdjudicationRequest {
  ...
  appendLog(
    state,
    `White Cell adjudication required: ${reason}`,
...
export function getPlayersBySide(
  state: GameState,
  side: "Blue" | "Red",
): PlayerState[] {
...
export function recordGameStartSummary(
  ...
  appendLog(
    draft,
    "Game-start state-of-world summary recorded.",
    ["SUMMARY_REQUIRED"],
    "public",
  );
  return draft;
...
export function validateState(state: GameState): RuleIssue[] {
  ...
  for (const player of entryValues(state.players)) {
    if (player.resource_points < 0 && !player.allow_deficit) {
      issues.push(
        makeIssue(
          `${player.id} has negative RPs without an explicit deficit exception.`,
  ...
  for (const force of entryValues(state.forces)) {
    ...
    if (
      force.owner === "US" &&
      force.readiness_level &&
      !READINESS_LEVELS.includes(force.readiness_level)
    ) {
      issues.push(
        makeIssue(`${force.id} has unsupported U.S. readiness.`, [
    ...
    if (force.pinned.active && force.reset_required) {
      issues.push(
        makeIssue(
          `${force.id} is both pinned and reset-required; White Cell should confirm availability.`,
          ["Validation:Reset"],
          ["WHITE_CELL_ADJUDICATION"],
          "adjudication",
        ),
  ...
  for (const item of state.ground_truth) {
    if (
      item.effect.visibility !== "public" &&
      item.visible_to.includes("Public")
    ) {
      issues.push(
        makeIssue(`${item.id} leaks a private outcome to Public.`, [
          "Validation:PrivateVisibility",
        ]),
      );
    }
  }
  for (const request of state.pending_adjudications.filter(
...
function phaseIssue(
  ...
  if (!expectedList.includes(state.phase)) {
    return makeIssue(
      `Expected phase ${expectedList.join(" or ")}, got ${state.phase}.`,
      ["Turn Sequence"],
    );
  }
  return undefined;
...
export function signalRedCards(
  ...
  if (cardIds.length > 3) {
    issues.push(
      makeIssue("Red may signal up to three cards.", [
        "5.2 Red Signaling Phase",
      ]),
    );
  }
  ...
  if (uniqueCardIds.length !== cardIds.length) {
    issues.push(
      makeIssue("Red signaled the same card more than once.", [
        "5.2 Red Signaling Phase",
      ]),
    );
  }
  ...
  for (const cardId of cardIds) {
    if (!deck.has(cardId)) {
      issues.push(
        makeIssue(`${cardId} is not in ${playerId}'s Action/Investment deck.`, [
          "6 Cards",
        ]),
      );
    }
  ...
  if (briefSummary.trim().length > 0) {
    appendLog(
      draft,
      `${playerId} intelligence briefing: ${briefSummary}`,
      ["SUMMARY_REQUIRED"],
      "public",
      {
        player_id: playerId,
...
export function beginBlueReadinessBill(state: GameState): {
  state: GameState;
  issues: RuleIssue[];
} {
  const issues: RuleIssue[] = [];
  if (!allRedPlayersSignaled(state)) {
    issues.push(
      makeIssue(
        "All Red players must complete signaling before Blue readiness.",
        ["5.2 Red Signaling Phase"],
      ),
    );
    return { state, issues };
...
export function setActiveBluePlayer(
  state: GameState,
  playerId: PlayerId,
): { state: GameState; issues: RuleIssue[] } {
  const draft = cloneState(state);
...
export function setUsForceReadiness(
  ...
  if (!READINESS_LEVELS.includes(readinessLevel)) {
    issues.push(
      makeIssue(`${readinessLevel}% is not a supported U.S. readiness level.`, [
        "15 U.S. Readiness",
      ]),
    );
  }
...
function spendResources(
  ...
  if (!player) {
    if (amount === 0) {
      appendLog(
        state,
        `${playerId} resolved zero-cost controller action for ${reason}.`,
        ["DETERMINISTIC"],
        "public",
        {
          player_id: playerId,
  ...
  if (
    ...
    !allowDeficit &&
    !player.allow_deficit
  ) {
    return makeIssue(`${playerId} lacks ${amount} RP for ${reason}.`, [
  ...
  appendLog(
    state,
    `${playerId} paid ${amount} RP for ${reason}.`,
    ["DETERMINISTIC"],
    "public",
    { player_id: playerId },
  );
...
export function payUsReadinessBill(state: GameState): {
  state: GameState;
  bill: ReadinessBill;
  issues: RuleIssue[];
} {
  ...
  for (const issue of bill.issues.filter(
    (entry) => entry.severity === "adjudication",
  )) {
    addAdjudication(
      draft,
      issue.message,
  ...
  if (
    issues.some(
      (entry) =>
        entry.severity === "error" || entry.severity === "adjudication",
    )
  ) {
...
export function advanceBlueToActions(state: GameState): {
  state: GameState;
  issues: RuleIssue[];
} {
  const draft = cloneState(state);
  ...
  appendLog(
    draft,
    "Blue investments complete. Blue actions may now be resolved.",
    ["DETERMINISTIC"],
    "public",
  );
  return { state: draft, issues };
...
function cardBaseCost(card: Card): RuleValue<number> {
  ...
  if (
    card.cost.resource_points.requires_adjudication ||
    card.cost.resource_points.expression
  ) {
    return {
      ok: false,
      issue: makeIssue(
  ...
  return {
    ok: false,
    issue: makeIssue(
      `${card.id} has an unsupported cost reference.`,
      ["6.2 Card Data Schema"],
      ["CARD_DEFINED"],
      "adjudication",
    ),
...
function redAdditionalCardCost(
  state: GameState,
  playerId: PlayerId,
  card: Card,
): number {
...
function validateCardFrequency(
  state: GameState,
  ...
  if (
    frequency.length === 0 ||
    frequency.includes("may play every turn") ||
    frequency.includes("can be played each turn")
  ) {
    return undefined;
  }
  ...
  if (frequency.includes("once per game") && playerHistory.length > 0) {
    return makeIssue(
      `${card.id} may only be played once per game by ${playerId}.`,
      ["6 Cards"],
    );
  }
  if (frequency.includes("once per two turns")) {
    const lastPlayed = [...playerHistory].sort(
      (left, right) => right.turn - left.turn,
    )[0];
    if (lastPlayed && state.turn - lastPlayed.turn < 2) {
      return makeIssue(
        `${card.id} may only be played once per two turns by ${playerId}.`,
        ["6 Cards"],
      );
    }
  }
...
function matchingOutcomeRows(
  ...
  return card.resolution.outcome_map.filter(
    (row) => row.outcome === outcome || row.label === outcome,
  );
}
...
function setPlayerResourceAllocation(
  state: GameState,
  playerId: PlayerId,
  value: JsonValue,
): void {
...
function applySingleEffect(
  ...
  if (
    trackDeferred &&
    (effect.visibility !== "public" || effect.timing !== "immediate")
  ) {
    addGroundTruthForEffect(
      state,
      effect,
      owner,
      "Tracked due to private, future, temporary, or conditional handling.",
    );
  }
  if (effect.requires_adjudication) {
    addAdjudication(
      state,
      `Effect ${effect.type} requires White Cell resolution.`,
      ["6.4 Effect Schema"],
      effect.value,
      owner,
    );
  ...
  switch (effect.type) {
    ...
    case "deploy_or_redeploy_forces":
      if (force && typeof effect.value === "string") {
        if (force.pinned.active) {
          return makeIssue(
            `${force.id} is pinned and may not move without White Cell confirmation.`,
            ["11.1 Pinned Forces"],
          );
        }
    ...
    case "set_readiness_level":
      if (force) {
        force.readiness_level = numberFrom(
          effect.value,
          force.readiness_level ?? 100,
        ) as ReadinessLevel;
      }
      break;
      ...
      if (force) {
        force.pinned = {
          ...
          remaining_turns: isRecord(effect.value)
            ? (numberFrom(effect.value.duration_turns, 1) as
                | number
                | "indefinite")
            : 1,
      ...
      if (isRecord(effect.value)) {
        const base: Base = {
          ...
          out_of_area_discount: numberFrom(
            effect.value.out_of_area_discount,
            0,
          ),
    ...
    case "create_proxy_force":
      if (isRecord(effect.value)) {
        const proxy: ProxyForce = {
          ...
          reliability: stringFrom(
            effect.value.reliability,
            "Medium",
          ) as ReliabilityLevel,
    ...
    case "procure_forces":
      if (isRecord(effect.value)) {
        ...
        setEntryValue(state.forces, id, {
          ...
          modernization_level: numberFrom(effect.value.modernization_level),
          location_id: stringFrom(effect.value.location_id),
          home_base_id: stringFrom(
            effect.value.home_base_id,
            stringFrom(effect.value.location_id),
          ),
          readiness_level:
    ...
    case "schedule_future_effect":
      ...
      break;
    case "expire_effect":
      for (const item of state.ground_truth.filter(
        (entry) => entry.id === effect.target,
      )) {
        item.status = "expired";
      }
      break;
  ...
  appendLog(
    state,
    `Applied effect ${effect.type} to ${effect.target}.`,
    ["CARD_DEFINED"],
    effect.visibility,
    {
      player_id: owner,
...
export function calculateCombatFactors(
  ...
  for (const commitment of commitments) {
    ...
    let note =
      commitment.source === "conus_reactive"
        ? "US CONUS reactive table"
        : "In-theater table";
    ...
    if (playerId === "US") {
      const readiness = force.readiness_level ?? 100;
      const readinessResult = applyReadinessImpact(
        baseCf,
        readiness,
        extensions,
      );
...
export function selectCrtAColumn(
  blueCfs: number,
  redCfs: number,
): RuleValue<CtrAColumn> {
  ...
  const ratio = Math.floor(
    Math.max(blueCfs, redCfs) / Math.min(blueCfs, redCfs),
  );
  if (ratio <= 1) {
    return { ok: true, value: "1:1" };
  }
...
export function selectRtBColumn(
  blueFfs: number,
  redFfs: number,
): RuleValue<RtBColumn> {
  ...
  const ratio = Math.floor(
    Math.max(blueFfs, redFfs) / Math.min(blueFfs, redFfs),
  );
  if (ratio < 2) {
  ...
  return {
    ok: true,
    value: blueFfs > redFfs ? "Blue Advantage" : "Red Advantage",
  };
}
...
function criticalCapabilityModifier(
  ...
  const bestBlue = Math.max(
    ...capabilities.map((capability) =>
      bestCapability(state, bluePlayers, capability),
    ),
  );
  const bestRed = Math.max(
    ...capabilities.map((capability) =>
      bestCapability(state, redPlayers, capability),
    ),
  );
  return bestBlue - bestRed;
}

function totalFfsForCommitments(
  state: GameState,
  commitments: ForceCommitment[],
): number {
  return commitments.reduce((sum, commitment) => {
    ...
    const ffs = commitment.out_of_area_arrival
      ? Math.ceil(force.force_factors / 2)
      : force.force_factors;
    return sum + ffs;
  }, 0);
}

function commitmentsOwnedBy(
  state: GameState,
  playerId: PlayerId,
  commitments: ForceCommitment[],
): ForceCommitment[] {
...
function resolveCrtA(
  ...
  const bluePlayers =
    args.blue_players ??
    getPlayersBySide(draft, "Blue").map((player) => player.id);
  const redPlayers =
    args.red_players ??
    getPlayersBySide(draft, "Red").map((player) => player.id);
  ...
  const blueResults = bluePlayers.map((playerId) =>
    calculateCombatFactors(
      draft,
      playerId,
      commitmentsOwnedBy(draft, playerId, args.blue_commitments ?? []),
    ),
  );
  const redResults = redPlayers.map((playerId) =>
    calculateCombatFactors(
      draft,
      playerId,
      commitmentsOwnedBy(draft, playerId, args.red_commitments ?? []),
    ),
  );
  ...
  const cardModifier = card.resolution.modifiers.reduce(
    (sum, modifier) => sum + (modifier.value ?? 0),
    0,
  );
  const capabilityModifier = criticalCapabilityModifier(
    draft,
    card.resolution.critical_capabilities,
    bluePlayers,
    redPlayers,
  );
...
function resolveRtB(
  ...
  if (!args.blue_commitments || !args.red_commitments) {
    const issue = makeIssue(
      `${card.id} requires force commitments for RT B.`,
      ["12 RT B"],
      ["WHITE_CELL_ADJUDICATION"],
      "adjudication",
    );
  ...
  const modifier =
    card.resolution.modifiers.reduce(
...
function resolveCardD10(
  ...
  if (!outcome) {
    ...
    addAdjudication(
      ...
      issue.message,
      issue.rule_refs,
      { card_id: card.id, roll_total: roll.total },
      args.acting_player_id,
      card.id,
    );
  }
  appendLog(
    draft,
    `${card.id} rolled ${roll.formula}${outcome ? ` => ${outcome}` : ""}.`,
    ["CARD_DEFINED"],
    visibilityForCard(card),
    {
  ...
  return {
    outcome: outcome as ProbabilityOutcome | "Success" | "Fail" | undefined,
    roll,
    issues,
  };
}
...
function applyCardOutcome(
  ...
  const effects =
    rows.length > 0
      ? [...card.effects, ...rows.flatMap((row) => row.effects)]
      : card.effects;
  for (const effect of effects) {
    const issue = applySingleEffect(
      draft,
      { ...effect, source_card_id: effect.source_card_id ?? card.id },
      actor,
    );
    if (issue) {
      issues.push(issue);
...
export function resolveCard(
  ...
  if (!cost.ok) {
    ...
    addAdjudication(
      draft,
      cost.issue.message,
      cost.issue.rule_refs,
      undefined,
      args.acting_player_id,
      card.id,
    );
  } else if (args.acting_player_id === "WhiteCell") {
    // no costs for white cell
  } else {
    const extraCost = redAdditionalCardCost(draft, args.acting_player_id, card);
    const spendIssue = spendResources(
      draft,
      args.acting_player_id,
      cost.value + extraCost,
      `playing ${card.id}`,
    );
    if (spendIssue) {
      issues.push(spendIssue);
  ...
  if (issues.some((issue) => issue.severity === "adjudication")) {
    return {
      state: draft,
      issues,
      adjudications: draft.pending_adjudications.filter(
        (entry) => entry.status === "pending",
      ),
    };
  }
  ...
  const method =
    card.resolution.method.find(
      (entry) => entry !== "white_cell_adjudication",
    ) ?? "white_cell_adjudication";

  if (method === "fixed_effect") {
    appendLog(
      draft,
      `${card.id} resolved as a fixed effect.`,
      ["CARD_DEFINED"],
      visibilityForCard(card),
      {
        player_id: args.acting_player_id,
    ...
    if (!reliability) {
      const issue = makeIssue(
        `${card.id} needs a proxy reliability level.`,
        ["16 Proxy Forces"],
        ["WHITE_CELL_ADJUDICATION"],
        "adjudication",
      );
      ...
      appendLog(
        draft,
        `${card.id} proxy reliability: ${proxyRoll.formula} => ${outcome}.`,
        ["DETERMINISTIC"],
        visibilityForCard(card),
        {
          player_id: args.acting_player_id,
    ...
    appendLog(
      draft,
      `${args.acting_player_id} played ${card.id}: ${card.title}.`,
      ["CARD_DEFINED"],
      visibilityForCard(card),
      {
        player_id: args.acting_player_id,
  ...
  if (!hasAnyIssue(issues)) {
    ...
    if (
      card.resolution.reset_rules_apply &&
      outcome &&
      outcome !== "Success" &&
      outcome !== "Fail"
    ) {
      ...
      if (card.resolution.pinning) {
        ...
        appendLog(
          draft,
          `${card.id} reset rules scheduled after pinning is removed.`,
          ["CARD_DEFINED"],
          visibilityForCard(card),
          {
            player_id: args.acting_player_id,
...
function redPlayedMixIssueForCardIds(
  state: GameState,
  playedIds: CardId[],
): RuleIssue | undefined {
...
function redPlayDeadEndIssue(
  ...
  const canStillCompleteMix = remainingIds.some(
    (remainingId) =>
      !redPlayedMixIssueForCardIds(state, [...playedIds, remainingId]),
  );
...
export function getRedChoiceOptions(
  state: GameState,
  playerId: PlayerId,
  ...
  return {
    remaining,
    canSkip: !redPlayedMixIssueForCardIds(state, play.played_card_ids),
  };
}
...
export function playRedSignaledCard(
  ...
  if (
    !signal ||
    !play ||
    !signal.card_ids.includes(cardId) ||
    play.played_card_ids.includes(cardId) ||
    play.skipped
  ) {
    return {
      state,
      issues: [
        makeIssue(
          `${cardId} is not a remaining signaled card for ${playerId}.`,
  ...
  const resolved = resolveCard(
    state,
    { ...args, acting_player_id: playerId, card_id: cardId },
    roller,
  );
  if (!resolved.issues.some((issue) => issue.severity === "error")) {
    ...
    appendLog(
      ...
      `${playerId} has remaining signaled options: ${
        getRedChoiceOptions(resolved.state, playerId)
...
export function skipRemainingRedCards(
  state: GameState,
  playerId: PlayerId,
): { state: GameState; issues: RuleIssue[] } {
  const draft = cloneState(state);
...
export function beginRedInvestmentsAndActions(
  ...
  if (sequence) {
    const redPlayers = getPlayersBySide(draft, "Red").map(
      (player) => player.id,
    );
    const uniqueSequence = new Set(sequence);
    const missing = redPlayers.filter(
      (playerId) => !uniqueSequence.has(playerId),
    );
    const unknown = sequence.filter(
      (playerId) => !redPlayers.includes(playerId),
    );
    if (
      uniqueSequence.size !== sequence.length ||
      missing.length > 0 ||
      unknown.length > 0
    ) {
      issues.push(
    ...
    if (!roller) {
      issues.push(
        makeIssue("Random Red player sequence requires a D10 roller.", [
          "17.4 Random Red Player Sequence",
        ]),
      );
      return { state, issues };
    ...
    const direction =
      draft.rules_in_effect.random_red_sequence_order ?? "ascending";
    const hasTie =
      new Set(rolls.map((entry) => entry.roll)).size !== rolls.length;
    if (
      hasTie &&
      draft.rules_in_effect.random_red_sequence_tie_rule !== "player_id"
    ) {
      addAdjudication(draft, "Random Red sequence has a tie.", [
        "17.4 Random Red Player Sequence",
      ]);
    }
  ...
  appendLog(
    draft,
    `Red Investments and Actions sequence: ${draft.red_sequence.join(", ")}.`,
    ["WHITE_CELL_ADJUDICATION"],
    "public",
  );
  return { state: draft, issues };
...
export function advanceRedActivePlayer(state: GameState): GameState {
  ...
  if (nextIndex >= draft.red_sequence.length) {
    ...
    appendLog(
      draft,
      `Active Red player is now ${draft.active_player_id}.`,
      ["DETERMINISTIC"],
      "public",
      {
...
export function calculateMovementCost(
  ...
  if (!from || !to) {
    return {
      cost: 0,
      issues: [
        makeIssue("Movement request references an unknown location.", [
  ...
  if (
    from.aor_id === to.aor_id ||
    request.from_location_id === request.to_location_id ||
    to.country_owner === request.player_id
  ) {
    return {
      cost: 0,
      issues,
      notes: ["Same AOR, same location, or own country movement is free."],
    };
  }
  ...
  if (forces.some((force) => force.home_base_id === request.to_location_id)) {
    return {
      cost: 0,
      issues,
      notes: ["Redeployment back to home base is free."],
    };
  }
  const totalFfs = forces.reduce((sum, force) => sum + force.force_factors, 0);
  if (
    request.player_id === "US" &&
    isConus(state, request.from_location_id) &&
    !isConus(state, request.to_location_id)
  ) {
    const table = getConusDeploymentCost(
      totalFfs,
      request.timing,
      state.rules_in_effect.table_extensions,
    );
    if (table.ok) {
      return {
        cost: table.value,
        issues,
        notes: [`U.S. CONUS ${request.timing} deployment table used.`],
      };
    }
  ...
  if (
    ...
    return {
      cost: 1,
      issues,
      notes: ["OCONUS-to-OCONUS redeployment costs 1 RP per movement."],
    };
  }
  issues.push(
    makeIssue(
...
export function buyBackReadiness(
  ...
  for (const forceId of forceIds) {
    ...
    if (targetReadiness <= current) {
      issues.push(
        makeIssue(
          `${forceId} is already at or above ${targetReadiness}% readiness.`,
          ["15.2 Readiness Buy-Back Cost"],
        ),
      );
      continue;
  ...
  for (const group of groups.values()) {
    const cost = getReadinessBuyBackCost(
      group.ffs,
      group.increase,
      group.location,
      draft.rules_in_effect.table_extensions,
    );
    if (cost.ok) {
...
export function procureForces(
  ...
  if (!player) {
    return {
      ...
      cost: 0,
      issues: [
        makeIssue(`Unknown player ${args.player_id}.`, ["13.1 Procurement"]),
      ],
    };
  }
  if (args.modernization_level > player.national_tech_level) {
    issues.push(
      makeIssue(
        "Procured force Mod Level may not exceed National Tech Level.",
        ["13.1 Procurement"],
      ),
    );
  }
  const cost = getProcurementCost(
    args.force_factors,
    args.modernization_level,
    draft.rules_in_effect.table_extensions,
  );
  if (!cost.ok) {
    issues.push(cost.issue);
...
export function modernizeForces(
  ...
  if (!player) {
    return {
      ...
      cost: 0,
      issues: [
        makeIssue(`Unknown player ${args.player_id}.`, ["13.2 Modernization"]),
      ],
    };
  }
  if (args.target_modernization_level > player.national_tech_level) {
    issues.push(
      makeIssue(
        "Modernized force Mod Level may not exceed National Tech Level.",
        ["13.2 Modernization"],
      ),
    );
  }
  ...
  if (args.percent_limit !== undefined) {
    const allowedFfs = Math.floor(
      ownedForces.reduce((sum, force) => sum + force.force_factors, 0) *
        args.percent_limit,
    );
    if (requestedFfs > allowedFfs) {
      issues.push(
        makeIssue("Modernization exceeds the card-defined percentage limit.", [
          "13.2 Modernization",
        ]),
      );
    }
  ...
  for (const force of requestedForces) {
    if (force.owner !== args.player_id) {
      issues.push(
        makeIssue(`${force.id} is not owned by ${args.player_id}.`, [
          "13.2 Modernization",
        ]),
      );
    }
    if (force.pinned.active) {
      issues.push(
        makeIssue("Pinned forces may not be modernized.", [
          "13.2 Modernization",
        ]),
      );
    }
    ...
    const increase =
      args.target_modernization_level - force.modernization_level;
    if (increase <= 0) {
      issues.push(
        makeIssue(`${force.id} is already at or above target Mod Level.`, [
          "13.2 Modernization",
        ]),
      );
      continue;
    }
    const cost = getModernizationCost(
      force.force_factors,
      increase,
      draft.rules_in_effect.table_extensions,
    );
  ...
  appendLog(
    draft,
    `${args.player_id} modernized ${requestedFfs} FF to M${args.target_modernization_level}.`,
    ["DETERMINISTIC"],
    "public",
    {
      player_id: args.player_id,
...
export function retireUsForces(
  state: GameState,
  forceIds: ForceId[],
): { state: GameState; issues: RuleIssue[] } {
  const draft = cloneState(state);
...
export function applyResetRules(
  ...
  if (resetFfs < requiredFfs) {
    issues.push(
      makeIssue(
        "Reset selection covers fewer than 50 percent of committed FFs.",
        ["11.3 Reset Rules"],
        ["WHITE_CELL_ADJUDICATION"],
        "adjudication",
      ),
  ...
  for (const force of committed.filter((entry) => entry.owner === "US")) {
    ...
    if (!READINESS_LEVELS.includes(next as ReadinessLevel)) {
      ...
      addAdjudication(
        draft,
        `Reset readiness reduction for ${force.id} needs White Cell resolution.`,
        ["11.3 Reset Rules"],
      );
    } else {
      force.readiness_level = next as ReadinessLevel;
  ...
  appendLog(
    draft,
    `Reset rules applied to ${resetFfs} FF; required ${requiredFfs} FF.`,
    ["DETERMINISTIC"],
    "public",
  );
  return { state: draft, issues };
...
export function restoreResetRedForces(
  state: GameState,
  playerId: PlayerId,
  forceIds: ForceId[],
): { state: GameState; cost: number; issues: RuleIssue[] } {
...
export function resolveProxyParticipation(
  ...
  if (!proxy) {
    return {
      state,
      issues: [makeIssue(`Unknown proxy ${proxyId}.`, ["16 Proxy Forces"])],
    };
  }
  ...
  appendLog(
    draft,
    `${proxy.label} proxy reliability ${roll.formula} => ${outcome}.`,
    ["DETERMINISTIC"],
    "public",
  );
...
export function developProxyForce(
  ...
  state: GameState;
  delivered_ffs: number;
  delivered_mod_level: number;
  issues: RuleIssue[];
} {
  const draft = cloneState(state);
  const baseCost = getProcurementCost(
    args.intended_ffs,
    args.intended_mod_level,
    draft.rules_in_effect.table_extensions,
  );
  if (!baseCost.ok) {
    return {
      state,
      delivered_ffs: 0,
      delivered_mod_level: 0,
      issues: [baseCost.issue],
    };
  }
  ...
  draft.rolls.push(roll);
  const success =
    getProxyReliabilityResult(roll.total, args.reliability) === "Success";
  const deliveredFfs = success
    ? args.intended_ffs
  ...
  if (proxy) {
    ...
    proxy.modernization_level = Math.max(
      proxy.modernization_level,
      deliveredMod,
    );
  }
  appendLog(
    draft,
    `Proxy development for ${args.proxy_id}: ${roll.formula}.`,
    ["DETERMINISTIC"],
    "private_to_player_and_white_cell",
    {
  ...
  return {
    state: draft,
    delivered_ffs: deliveredFfs,
    delivered_mod_level: deliveredMod,
    issues: [],
  };
}

export function annualResourceAllocation(
  state: GameState,
  roller: DiceRoller,
): { state: GameState; issues: RuleIssue[]; budgetRoll?: RollRecord } {
  const draft = cloneState(state);
  const issues: RuleIssue[] = [];
  ...
  appendLog(
    draft,
    `U.S. budget variation ${budgetRoll.formula}: ${usVariation >= 0 ? "+" : ""}${usVariation} RP.`,
    ["DETERMINISTIC"],
    "public",
    {
      player_id: "US",
...
export function recordStateOfWorldSummary(
  state: GameState,
  summary: string,
): GameState {
  const draft = cloneState(state);
  ...
  for (const pendingReset of draft.pending_resets.filter(
    ...
    if (!stillPinned) {
      const reset = applyResetRules(
        draft,
        pendingReset.force_ids,
        pendingReset.outcome,
      );
      draft.forces = reset.state.forces;
      draft.players = reset.state.players;
      draft.event_log = reset.state.event_log;
      draft.pending_adjudications = reset.state.pending_adjudications;
      draft.pending_resets = reset.state.pending_resets;
      const updated = draft.pending_resets.find(
        (entry) => entry.id === pendingReset.id,
      );
      if (updated) {
    ...
    if (
      ...
      if (issue?.severity === "adjudication") {
        addAdjudication(
          ...
          item.effect.value,
          undefined,
          item.source === "white_cell" || item.source === "rule_id"
            ? undefined
            : item.source,
        );
      }
...
export async function resolveWhiteCellAdjudication(
  ...
  if (!request) {
    return {
      state,
      issues: [
        makeIssue(`Unknown adjudication ${adjudicationId}.`, [
...
export function injectWhiteCellEvent(
  ...
  if (
    ...
    return {
      state,
      issues: [
        makeIssue(`${cardId} is not an event card.`, [
          "5 Turn Sequence",
          "6 Cards",
        ]),
      ],
  ...
  const result = resolveCard(
    state,
    { acting_player_id: "WhiteCell", card_id: cardId },
    roller,
  );
  if (!result.issues.some((issue) => issue.severity === "error")) {
    appendLog(
      result.state,
      `White Cell injected event ${cardId}: ${note}`,
      ["WHITE_CELL_ADJUDICATION"],
      visibilityForCard(card),
      {
        card_id: cardId,
